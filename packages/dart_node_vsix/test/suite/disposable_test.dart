...
void main() {
  ...
  suite('Disposable API', syncTest(() {
    suiteSetup(asyncTest(() async {
      await waitForExtensionActivation();
    }));

    test('createDisposable works correctly', syncTest(() {
      var disposed = false;
    ...
    test('createDisposable creates disposable', syncTest(() {
      var disposed = false;
      final disposable = createDisposable(() => disposed = true);
      assertOk(!disposed, 'Should not be disposed yet');
      disposable.dispose();
    ...
    test('Multiple disposables can be created', syncTest(() {
      var count = 0;
