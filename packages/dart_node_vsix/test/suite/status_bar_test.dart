...
void main() {
  ...
  suite('Status Bar API', syncTest(() {
    suiteSetup(asyncTest(() async {
      await waitForExtensionActivation();
    }));

    test('Status bar item is created with correct text', syncTest(() {
      final api = getTestAPI();
      final text = api.getStatusBarText();
      assertOk(text.contains('VSIX Test'), 'Status bar should have test text');
    }));

    test('StatusBarAlignment enum has correct values', syncTest(() {
      assertEqual(StatusBarAlignment.left.value, 1);
      assertEqual(StatusBarAlignment.right.value, 2);
    }));

    test('createStatusBarItem creates item', syncTest(() {
    ...
    test('Status bar item tooltip can be set', syncTest(() {
      final item = vscode.window.createStatusBarItem()..tooltip = 'My Tooltip';
      assertEqual(item.tooltip, 'My Tooltip');
      item.dispose();
    }));

    test('Status bar item command can be set', syncTest(() {
      final item = vscode.window.createStatusBarItem()
