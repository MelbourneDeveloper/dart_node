/// Tests for JSX DSL (operator-based element composition).
@TestOn('js')
library;
...
void main() {
  ...
  test('key parameter works for list items', () {
    ...
    final result = render(fc(component));
    final list = result.getByTestId('keyed-list');
    expect(list.textContent, contains('First'));
    expect(list.textContent, contains('Second'));
    expect(list.textContent, contains('Third'));
    result.unmount();
  });
  ...
  test('spread props are applied to elements', () {
    ...
    final result = render(fc(component));
    final el = result.getByTestId('spread-test');
    expect(el.className, equals('explicit-class'));
    result.unmount();
  });
  ...
  test('explicit props override spread props', () {
    ...
    final result = render(fc(component));
    final el = result.getByTestId('override-test');
    // explicit className should override spread
    expect(el.className, equals('overridden'));
    result.unmount();
  });
}
