...
void main() {
  ...
  suite(
    'Command Integration - Dialog Mocking',
    syncTest(() {
      ...
      test(
        'deleteLock command with LockTreeItem - confirmed',
        asyncTest(() async {
          _log(
            '[CMD DIALOG] Running: deleteLock with LockTreeItem - confirmed',
          );
          final api = getTestAPI();
          final key = agentKey;
      ...
      test(
        'deleteLock command with AgentTreeItem - confirmed',
        asyncTest(() async {
          _log(
            '[CMD DIALOG] Running: deleteLock with AgentTreeItem - confirmed',
          );
          final api = getTestAPI();
          final key = agentKey;
          if (key == null) throw StateError('agentKey not set');
          ...
          _log(
            '[CMD DIALOG] PASSED: deleteLock with AgentTreeItem - confirmed',
          );
        }),
      );

      test(
        'deleteLock command - no filePath shows error',
        asyncTest(() async {
      ...
      test(
        'deleteLock command - cancelled does nothing',
        asyncTest(() async {
      ...
      test(
        ...
        asyncTest(() async {
          ...
          final result = await api
              .callTool('register', createArgs({'name': targetName}))
              .toDart;
      ...
      test(
        'deleteAgent command - no agentName shows error',
        asyncTest(() async {
      ...
      test(
        'deleteAgent command - cancelled does nothing',
        asyncTest(() async {
      ...
      test(
        'sendMessage command - with target agent',
        asyncTest(() async {
      ...
      test(
        'sendMessage command - without target uses quickpick',
        asyncTest(() async {
          _log(
            '[CMD DIALOG] Running: sendMessage - without target uses quickpick',
          );
          final api = getTestAPI();
          ...
          _log(
            '[CMD DIALOG] PASSED: sendMessage - without target uses quickpick',
          );
        }),
      );

      test(
        'sendMessage command - broadcast to all',
        asyncTest(() async {
      ...
      test(
        'sendMessage command - cancelled at recipient selection',
        asyncTest(() async {
          _log(
            '[CMD DIALOG] Running: sendMessage - cancelled at recipient '
            'selection',
          );
          ...
          _log(
            '[CMD DIALOG] PASSED: sendMessage - cancelled at recipient '
            'selection',
          );
        }),
      );
      ...
      test(
        'sendMessage command - cancelled at sender input',
        asyncTest(() async {
      ...
      test(
        'sendMessage command - cancelled at message input',
        asyncTest(() async {
          _log(
            '[CMD DIALOG] Running: sendMessage - cancelled at message input',
          );
          final api = getTestAPI();
