...
void main() {
  ...
  suite('MCP Integration - UI Verification', syncTest(() {
    ...
    test('Empty state shows empty trees', asyncTest(() async {
      ...
      for (var i = 0; i < messagesTree.length; i++) {
        if (_getLabel(messagesTree[i]) == 'No messages') hasNoMessages = true;
      }
    ...
    test('Register agent-1 → label APPEARS in agents tree', asyncTest(() async {
      ...
      assertEqual(_getLabel(agentItem!), agent1Name,
    ...
    test('Register agent-2 → both agents visible in tree',
        asyncTest(() async {
    ...
    test('Acquire lock on /src/main.ts → file path APPEARS in locks tree',
        asyncTest(() async {
    ...
    test('Acquire 2 more locks → all 3 file paths visible',
      ...
      assertOk(api.findLockInTree('/src/main.ts') != null,
          '/src/main.ts MUST be in tree');
      assertOk(api.findLockInTree('/src/utils.ts') != null,
          '/src/utils.ts MUST be in tree');
      assertOk(api.findLockInTree('/src/types.ts') != null,
    ...
    test('Release /src/utils.ts → file path DISAPPEARS from tree',
        asyncTest(() async {
      ...
      assertEqual(api.findLockInTree('/src/utils.ts'), null,
    ...
    test('Update plan for agent-1 → plan content APPEARS in agent children',
        asyncTest(() async {
    ...
    test('Send 2 more messages → all 3 messages visible with correct labels',
      ...
      assertOk(api.findMessageInTree('Starting work') != null,
          'First message MUST be in tree');
      assertOk(api.findMessageInTree('Acknowledged') != null,
          'Second message MUST be in tree');
      assertOk(api.findMessageInTree('Done with main') != null,
    ...
    test('Broadcast message to * → message APPEARS in tree with "all" label',
        asyncTest(() async {
    ...
    test('Refresh syncs all state from server', asyncTest(() async {
      ...
      assertOk(msgTreeLen >= 4,
          'At least 4 messages in tree (including broadcast), got $msgTreeLen');

      final agentItem = api.findAgentInTree(agent1Name);
      assertOk(agentItem != null && _hasChildWithLabel(agentItem, 'Goal:'),
          'Agent should have plan child');

      _log('[MCP UI] refresh test PASSED');
    }));

    test('Disconnect clears all tree views', asyncTest(() async {
    ...
    test('Reconnect restores all state and tree views', asyncTest(() async {
      ...
      if (api.findAgentInTree(agent1Name) == null) {
        final result1 = await api.callTool('register',
            createArgs({'name': agent1Name})).toDart;
        agent1Key = extractKeyFromResult(result1.toDart);
      }
      if (api.findAgentInTree(agent2Name) == null) {
        final result2 = await api.callTool('register',
            createArgs({'name': agent2Name})).toDart;
      ...
      final hasPlan = agentItemForPlan != null &&
      ...
      assertOk(api.findAgentInTree(agent1Name) != null,
          '$agent1Name in tree');
      assertOk(api.findAgentInTree(agent2Name) != null,
          '$agent2Name in tree');
      assertOk(api.findLockInTree('/src/main.ts') != null,
      ...
      assertOk(agent1AfterReconnect != null &&
  ...
  suite('MCP Integration - Admin Operations', syncTest(() {
    ...
    test('CRITICAL: Admin tool must exist on server', asyncTest(() async {
      ...
      try {
        final result = await api.callTool('admin', createArgs({
          'action': 'delete_lock',
        ...
        assertOk(resultStr.contains('deleted') || resultStr.contains('error'),
    ...
    test('Setup: Connect and register agents', asyncTest(() async {
      _log('[MCP ADMIN] Running setup test');
      final api = getTestAPI();

      // Register admin agent
      final result1 = await api.callTool('register',
          createArgs({'name': adminAgentName})).toDart;
      ...
      final result2 = await api.callTool('register',
          createArgs({'name': targetAgentName})).toDart;
    ...
    test('Force release lock via admin → lock DISAPPEARS', asyncTest(() async {
      _log('[MCP ADMIN] Running force release test');
      final api = getTestAPI();

      assertOk(api.findLockInTree('/admin/test/file.ts') != null,
          'Lock should exist before force release');

      await api.callTool('admin', createArgs({
        'action': 'delete_lock',
        'file_path': '/admin/test/file.ts',
      })).toDart;
    ...
    test('Delete agent via admin → agent DISAPPEARS from tree',
      ...
      assertEqual(api.findAgentInTree(targetAgentName), null,
          'Target agent should be gone after delete');

      _log('[MCP ADMIN] delete agent test PASSED');
    }));

    test('Lock renewal extends expiration', asyncTest(() async {
    ...
    test('Mark message as read updates state', asyncTest(() async {
      _log('[MCP ADMIN] Running mark message as read test');
      final api = getTestAPI();

      // Send a message to admin agent
      final senderName = 'sender-$testId';
      final senderResult = await api.callTool('register',
          createArgs({'name': senderName})).toDart;
      ...
      final getResult = await api.callTool('message', createArgs({
        'action': 'get',
        'agent_name': adminAgentName,
        'agent_key': adminAgentKey,
      })).toDart;
      ...
      assertOk(api.findMessageInTree('Test message for read') != null,
  ...
  // ==========================================================================
  suite('MCP Integration - Lock State', syncTest(() {
    var agentKey = '';
    final testId = _dateNow();
    ...
    test('Setup: Connect and register agent', asyncTest(() async {
      ...
      final result = await api.callTool('register',
          createArgs({'name': agentName})).toDart;
    ...
    test('Lock on file creates decoration data in state', asyncTest(() async {
      ...
      final lockAgentName = _reflectGet(foundLock!, 'agentName')?.toString();
      assertEqual(lockAgentName, agentName, 'Lock should have correct agent');

      final lockReason = _reflectGet(foundLock, 'reason')?.toString();
      assertEqual(lockReason, 'Testing decorations',
    ...
    test('Lock without reason still works', asyncTest(() async {
      ...
      assertOk(lockReason == null || lockReason.isUndefinedOrNull,
    ...
    test('Active and expired locks computed correctly', asyncTest(() async {
      ...
      assertOk(agentLocks.length >= 1, 'Agent should have at least one lock');
      ...
      for (var i = 0; i < agentLocks.length; i++) {
        ...
        assertOk(expiresAtNum > _dateNow(),
    ...
    test('Release lock removes decoration data', asyncTest(() async {
  ...
  // ==========================================================================
  suite('MCP Integration - Tree Provider Edge Cases', syncTest(() {
    var agentKey = '';
    final testId = _dateNow();
    ...
    test('Long message content is truncated in tree', asyncTest(() async {
      ...
      assertOk(desc != null && desc.contains('AAA'),
          'Description should contain content');

      _log('[MCP EDGE] long message test PASSED');
    }));

    test('Long plan task is truncated in tree', asyncTest(() async {
    ...
    test('Agent with multiple locks shows all locks', asyncTest(() async {
      ...
      final lockCount = _countChildrenMatching(agentItem,
    ...
    test('Agent description shows lock and message counts', asyncTest(() async {
  ...
  // ==========================================================================
  suite('MCP Integration - Store Methods', syncTest(() {
    var storeAgentKey = '';
    final testId = _dateNow();
    ...
    test('Setup: Connect and register agents', asyncTest(() async {
      ...
      final result = await api.callTool('register',
          createArgs({'name': storeAgentName})).toDart;
    ...
    test('store.forceReleaseLock removes lock', asyncTest(() async {
      ...
      assertEqual(api.findLockInTree('/store/force/release.ts'), null,
          'Lock should be removed by forceReleaseLock');

      _log('[MCP STORE] forceReleaseLock test PASSED');
    }));

    test('store.deleteAgent removes agent and their data', asyncTest(() async {
      _log('[MCP STORE] Running deleteAgent test');
      final api = getTestAPI();

      // Register a target agent to delete
      final result = await api.callTool('register',
          createArgs({'name': targetAgentForDelete})).toDart;
      ...
      assertEqual(api.findAgentInTree(targetAgentForDelete), null,
          'Agent should be removed by deleteAgent');

      // Lock should also be gone (cascade delete)
      assertEqual(api.findLockInTree('/store/delete/agent.ts'), null,
          'Agent locks should be removed when agent is deleted');

      _log('[MCP STORE] deleteAgent test PASSED');
    }));

    test('store.sendMessage sends message via registered agent',
        asyncTest(() async {
      ...
      await api.callTool('register',
          createArgs({'name': recipientName})).toDart;

      // Use store method to send message (it registers sender automatically)
      final senderName = 'ui-sender-$testId';
      await api.sendMessage(senderName, recipientName,
          'Message from store.sendMessage').toDart;
