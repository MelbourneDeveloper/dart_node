...
void main() {
  ...
  suite('Extension Activation', syncTest(() {
    ...
    test('TestAPI has all required methods', syncTest(() {
    ...
    test('Initial state is disconnected', syncTest(() {
    ...
    test('Initial state has empty arrays', syncTest(() {
    ...
    test('Initial computed values are zero', syncTest(() {
  ...
  /// won't work.
  suite('MCP Server Feature Verification', syncTest(() {
    ...
    test('CRITICAL: Admin tool MUST exist on MCP server', asyncTest(() async {
      _log('[MCP FEATURE] Testing admin tool existence...');
      final api = getTestAPI();
      assertOk(agentKey.isNotEmpty, 'Should have agent key from suiteSetup');
    ...
    test(
      ...
      asyncTest(() async {
        ...
        try {
          ...
          final result = await api.callTool('subscribe', subscribeArgs).toDart;
    ...
    test('All core tools are available', asyncTest(() async {
