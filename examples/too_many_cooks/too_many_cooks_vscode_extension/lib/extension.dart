...
// Test infrastructure - accessed via JS interop from test harness
...
/// Test workspace folder set via globalThis for testing.
@JS('globalThis._tooManyCooksTestWorkspace')
external JSString? get _testWorkspaceFolder;

/// Get workspace folders from VSCode API.
@JS('vscode.workspace.workspaceFolders')
external JSArray<_WorkspaceFolder>? get _workspaceFolders;

/// WorkspaceFolder type from VSCode API.
extension type _WorkspaceFolder._(JSObject _) implements JSObject {
  external _Uri get uri;
}

/// Uri type from VSCode API.
extension type _Uri._(JSObject _) implements JSObject {
...
JSObject _doActivateSync(ExtensionContext context) {
  ...
  // Get workspace folder for database location
  // Check test workspace first, then VSCode workspace folders
  final testWs = _testWorkspaceFolder?.toDart;
  final folders = _workspaceFolders;
  final workspaceFolder =
      testWs ??
      (folders != null && folders.length > 0 ? folders[0].uri.fsPath : '.');
  _log('Using workspace folder: $workspaceFolder');
  _log('Database will be at: $workspaceFolder/.too_many_cooks/data.db');
  ...
  if (autoConnect) {
    ...
    // Use async IIFE to catch all errors including sync throws
    unawaited(
      Future<void>(() async {
        try {
          _log('Auto-connect: calling connect()...');
          await _storeManager?.connect();
          _log('Auto-connect: connect() returned');
          _log('Auto-connect: SUCCESS');
...
void _registerCommands(ExtensionContext context) {
  ...
  final sendMessageCmd = vscode.commands.registerCommandWithArgs<TreeItem?>(
    ...
    (item) async {
      ...
      if (toAgent == null) {
        _consoleLog('[EXT sendMessage] No target, showing agent picker...');
...
class _TestAPIImpl {
  ...
  Future<String> callTool(String name, Map<String, Object?> args) async {
    ...
    if (manager == null) return '{"error":"Not connected"}';
    return manager.callTool(name, args);
  }

  /// Bridge for JS: takes JSString name and JSObject args, returns JSPromise.
  /// Returns `JSPromise<JSAny?>` to avoid dart2js cross-module type check
  /// failures with generic type parameters like `JSPromise<JSString>`.
  JSPromise<JSAny?> _callToolForJS(JSString nameJs, JSObject argsJs) =>
      Future<JSAny?>(() async {
        try {
          final name = nameJs.toDart;
          // Convert JSObject args to Dart Map using dartify
          final rawArgs = argsJs.dartify();
          final dartArgs = <String, Object?>{
            if (rawArgs is Map)
              for (final e in rawArgs.entries) '${e.key}': e.value,
          };
          final result = await callTool(name, dartArgs);
          // Return as JSAny? (untyped) to avoid cross-module type issues
          return result.toJS as JSAny?;
        } on Object catch (e) {
          return '{"error":"callTool error: $e"}'.toJS as JSAny?;
        }
      }).toJS;
  ...
  void deleteAgent(String agentName) => _storeManager?.deleteAgent(agentName);
  ...
  JSObject toJS() {
    ...
    // callTool: direct async export avoids dart2js cross-module crashes
    // by keeping all type conversions within the extension module.
    _setProp(obj, 'callTool', _callToolForJS.toJS);
    // Sync methods wrapped to return Promises (TestAPI expects JSPromise)
    _setProp(
      obj,
      'forceReleaseLock',
      ((JSString filePath) {
        forceReleaseLock(filePath.toDart);
    ...
    _setProp(
      ...
      ((JSString fromAgent, JSString toAgent, JSString content) {
        sendMessage(fromAgent.toDart, toAgent.toDart, content.toDart);
        return Future<void>.value().toJS;
      }).toJS,
    );
