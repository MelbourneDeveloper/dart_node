...
(function() {
  ...
  // Ensure Mocha TDD globals are available on globalThis for dart2js @JS() bindings
  // These are set by index.js via mocha.suite.emit('pre-require', globalThis, ...)
  // but dart2js may resolve @JS() bindings at load time before they're set.
  // This check happens at runtime when the test file loads.
  if (typeof globalThis.suite !== 'function') {
    console.error('[WRAP-TESTS] WARNING: Mocha TDD globals not found on globalThis');
    console.error('[WRAP-TESTS] Make sure tests run via "npm test" (VSCode test runner)');
  }
